...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['password']) && !empty($_POST['password']) && isset($_POST['confirm_password']) && !empty($_POST['confirm_password'])) {
        ...
        // Retrieve old hashed password from the database
        $sql_select = "SELECT password FROM form WHERE email = ?";
        ...
        if (password_verify($password, $old_hashed_password)) {
            $updatePasswordMessage = "You cannot use your old password. <br> please enter new password";
